...
function (io) {
    ...
    socket.emit(method, json, function afterEmitted (result) {
      ...
      if (result && typeof result === 'string') {
        try {
          ...
          return cb(new Error("Server response could not be parsed!\n" + result));
        }
      }

      // TODO: Handle errors more effectively
      if (parsedResult === 404) return cb("404: Not found");
      if (parsedResult === 403) return cb("403: Forbidden");
      if (parsedResult === 500) return cb("500: Server error");
  ...
  // Return `io` to allow for simpler usage
  return io;
};
